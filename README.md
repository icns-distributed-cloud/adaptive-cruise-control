...
1. 객체 인식
    ...
    - YOLOv5 모델 중 가장 초당 프레임 수 가 높은 YOLOv5s에 커스텀 데이터셋을 학습
    - 라즈베리파이에 부착된 웹캠을 통해 실시간으로 전방 차량 인식
  
2. 거리측정
    - 객체 인식 시 나타나는  bound box의 좌표값을 추출하여 대상과의 거리가 1m 일 때 bound box의 높이와 너비값을 측정
    - 이후 인식된 객체의 bound box 높이와 너비값과 1m 일 때의 bound box 높이와 너비값의 비례식을 통해 거리를 측정
...
* **거리유지 기능 실험 결과**
  - 기준 주행 속도는 차량 키트가 스스로 움직일 수 있는 최저 속도로 설정하였다.<br>
   테스트 결과 거리가 1m에 가까워 지면 상당히 속도가 줄어들었고 70cm에 이르면 차량 키트가 완전히 정지하였으며<br> 
   전방에 가까운 차량이 없으면 원래의 주행 속도로 돌아오는 기능 또한 정상적으로 동작함을 확인 할 수 있었다
